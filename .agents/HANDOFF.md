...
- Each unbounded epoch shuffles all 720 scenarios into 45 batches of 16. Every worker collects one stochastic trajectory, and each batch receives four optimizer update passes against its fixed rollout log-probabilities.
...
- The reward is `0.02 * ego_progress_delta - 1.0 * ego_collision`. An overtake is classified when the ego center is at least one vehicle length (`0.58 m`) ahead in wrapped Frenet progress.
