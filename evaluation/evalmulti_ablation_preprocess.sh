...
if [[ "$PREPROCESSING" != "linear" && "$PREPROCESSING" != "no_normalization" ]]; then
    echo "Preprocessing must be linear or no_normalization: $PREPROCESSING" >&2
    exit 1
fi
if [[ -z "$OUTPUT_ROOT" ]]; then
    OUTPUT_ROOT="eval_results/preprocess_ablation_$PREPROCESSING"
fi
if [[ "$RENDER" == true ]]; then
    RENDER_FLAG="--render"
else
...
expected_directory="ckp_ablation_$PREPROCESSING"
for preprocessing in linear no_normalization; do
    checkpoint_directory="ckp_ablation_$preprocessing"
    if [[ "/$CHECKPOINT_PATH/" == *"/$checkpoint_directory/"* \
        && "$checkpoint_directory" != "$expected_directory" ]]; then
        echo "Preprocessing $PREPROCESSING requires a checkpoint from $expected_directory, not $checkpoint_directory" >&2
        exit 1
    fi
done
    ...
    for ego_idx in "${ego_indices[@]}"; do
        for opponent_raceline in "${OPPONENT_RACELINES[@]}"; do
            for opponent_speed_scale in "${OPPONENT_SPEED_SCALES[@]}"; do
                ...
                (
                    python -m evaluation.evalmulti_ablation_preprocess \
                        --map_name "$map_name" \
