...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Evaluate an End2Race preprocessing ablation on single-agent laps")
    # Model and artifacts
    ...
    if args.checkpoint_path is None:
        args.checkpoint_path = Path("checkpoint") / f"ckp_ablation_{args.preprocessing}" / "epoch_00500.pt"
    if args.output_dir is None:
        args.output_dir = Path(f"eval_results/preprocess_ablation_{args.preprocessing}")

    known_directories = {
        f"ckp_ablation_{mode}" for mode in End2Race.PREPROCESSING_OPTIONS
    }
    expected_directory = f"ckp_ablation_{args.preprocessing}"
    conflicting_directories = (
        known_directories.intersection(args.checkpoint_path.parts)
        - {expected_directory}
    )
    if conflicting_directories:
        parser.error(
            f"--preprocessing {args.preprocessing} requires a checkpoint from "
            f"{expected_directory}, not {sorted(conflicting_directories)[0]}"
        )
    return args
