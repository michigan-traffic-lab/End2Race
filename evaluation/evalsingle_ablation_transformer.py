...
def parse_arguments():
    ...
    parser.add_argument("--checkpoint_path", type=Path, default=Path("checkpoint/ckp_ablation_transformer/epoch_00500.pt"))
    parser.add_argument("--output_dir", type=Path, default=Path("eval_results/transformer_ablation"))
    ...
    while not done and lap_count < args.lap_num:
        if control_step == 0:
            lidar = mask_lidar_points(
                downsample_lidar(
                    obs["scans"][0], target_points=End2RaceTransformer.NUM_LIDAR_FEATURES
                ),
                args.noise,
                rng,
            )
            ...
            with torch.no_grad():
                ...
                actions, context = model(
                    lidar_tensor, speed_tensor, context
                )
