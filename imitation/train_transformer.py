...
class SequenceDataset(Dataset):
    def __init__(self, data_path: str | Path):
        ...
        print(f"Loaded {len(self.episodes)} episodes as {len(self.windows)} windows")
    ...
    def _load_episodes(self, data_path: str | Path):
        ...
        for csv_file in csv_files:
            ...
            self._create_windows(lidar_data, speed_data, action_data)
    ...
    def _create_windows(
        ...
        # Windows are stored as offsets, because materialising every one of them would
        # keep about forty copies of each episode in memory.
        for start_idx in range(
            0, len(lidar_data) - self.sequence_length + 1, SEQUENCE_STRIDE
        ):
    ...
    def __getitem__(self, index: int) -> tuple[torch.Tensor, ...]:
        ...
        return (
            ...
            torch.from_numpy(self._loss_mask(start_idx)),
        )
...
def train_epoch(
    ...
    for lidar_seq, speed_seq, target_actions, loss_mask in train_loader:
        ...
        steering_loss = (squared_error[:, 0] * weights).sum() / num_supervised
        weighted_speed_loss = speed_loss_weight * (
            (squared_error[:, 1] * weights).sum() / num_supervised
        )
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            f"Epoch {epoch}/{args.epochs}, loss: {total_loss:.5f}, "
