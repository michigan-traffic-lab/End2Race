...
- A launch trains independent models until stopped. Each model starts from the same input checkpoint with fresh PPO state, runs for 500 epochs, and is followed by the next model. Stochastic collection uses fixed steering and speed standard deviations of `0.05` and `0.50`.
...
PPO saves `config.json`, `checkpoints.json`, `episodes.jsonl`, `metrics.jsonl`, and qualifying policy checkpoints inside `checkpoint/ppo/`. Every deterministic full-pool evaluation with safety above 90% and an overtake rate above 60% saves another checkpoint in qualification order: `ppo_001.pt`, `ppo_002.pt`, and so on. `checkpoints.json` is a JSON array updated after each save with the model number, epoch, learning rate, screening results, rollout metrics, and PPO diagnostics. `episodes.jsonl` and `metrics.jsonl` identify every record by model and epoch. Each launch starts with a clean `checkpoint/ppo/` directory.
