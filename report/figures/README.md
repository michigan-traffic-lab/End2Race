...
`fig1` through `fig5` directories. Figures 3 and 5 retain only their selected
final frames; render videos and decoded frame sequences are temporary outputs.
...
- Overtake 1: ego `0`, opponent `raceline1`; frames `280,350,420`, reference
  ...
  x=`620`.
- Overtake 2: ego `1362`, opponent `raceline1`; frames `200,310,390`, reference
  ...
  at x=`82`.
- Following: ego `550`, opponent `raceline2`; frames `650,750,900`, reference
  ...
  at x=`74`.
- Collision: ego `236`, opponent `raceline1`; frames `301,349,385`, reference
...
process; the repository renderer remains at its defaults. Copy only the named
final PNG panels into `fig3` or `fig5` and discard the video and decoded frames.
