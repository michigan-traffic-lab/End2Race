...
\begin{figure*}[!t]
    ...
    \end{minipage}
    \caption{Head-to-head scenarios on the Austin circuit. Blue and red denote the ego vehicle and opponent, respectively, with their trajectories rendered in matching colors.}
...
Figure~\ref{fig:scenario-examples} illustrates four cases that highlight the model's behavior during overtaking encounters. All examples are rollouts generated by the PPO policy on the Austin circuit, focusing on the most challenging configuration ($r_{\mathrm{opp}}=\text{center}$ and $\alpha_{\mathrm{opp}}=0.8$), where lateral clearance and the speed difference between vehicles are minimized.
...
In this work, we introduce End2Race, an end-to-end learning framework for head-to-head autonomous racing. Our proposed policy achieves low inference latency and delivers competitive racing performance. Future work will focus on addressing the sim-to-real gap, deploying the learned policy to a physical vehicle, and competing against real opponents in F1Tenth competitions.
