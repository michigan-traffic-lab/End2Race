...
\begin{tabularx}{\textwidth}{l|YY|YY|YY|YY}
\hline
...
\hline
Dreamer~\cite{9811650} & 93.8 & 6.8 & 96.5 & 12.9 & 96.9 & 11.3 & 98.2 & 13.9 \\
...
We evaluate End2Race on the training and three novel test tracks, comparing it against learning-based methods from the literature. Evaluations of prior methods use their best-performing released checkpoints. Experiments cover two racing setups: single-vehicle timed trials and interactive head-to-head scenarios. In addition to performance metrics, trajectory analysis examines representative policy behaviors.
...
We first conduct single-vehicle timed trials to benchmark each model's high-speed navigation capability, reporting lap times and mean speeds in Table~\ref{tab:single_vehicle}. Among prior methods, Zhang--Loidl completes only one track and collides on the rest. Dreamer finishes all four tracks without collisions, but operates at low racing speeds around 3.0~m/s. TinyLidarNet is collision-free and notably faster, yet remains slower than our map-based expert.

Compared to the expert, which relies on explicit track knowledge and an optimized reference raceline, our map-free BC policy achieves comparable lap times and mean speeds. The PPO policy improves performance even further, setting the fastest overall pace. We attribute these gains to continued interactive exploration, through which the policy discovers behaviors that align with principles established in motorsport driving~\cite{mitchell2004driving}. One example is corner-exit track-out, in which the vehicle moves from the inside to the outside of a curve to reduce speed loss. Another example is corner-exit acceleration and corner-entry braking, in which the vehicle accelerates immediately upon exiting a curve, maintains high speed along the following straight, and brakes sharply just before entering the next curve.
...
Table~\ref{tab:ftg_unseen} reports aggregate performance across the three novel tracks. All three methods maintain safety rates above 90\%, with the map-based expert reaching the highest rate at 93.9\%. However, overtaking rates for the expert and BC drop substantially because they cannot catch up to the fast opponent at the 0.9 speed scale. Meanwhile, PPO achieves a high overtake rate due to its much higher speed. These results demonstrate that our learned policy generalizes to both novel tracks and unseen opponent behaviors.
